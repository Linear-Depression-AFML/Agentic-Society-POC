...
def run_game(initial_wealth, num_players, multiplier, game_number, df, rounds, env_params):
    ...
    for round_num in range(rounds):
        ...
        for pid in players:
            ...
            # Add specific trust scores for every other player
            for other_pid in players:
                c_col = f"CTrust_in_P{other_pid}"
                b_col = f"BTrust_in_P{other_pid}"
                
                if pid == other_pid:
                    # A player does not track trust in themselves (set to None or NaN)
                    new_row[c_col] = None
                    new_row[b_col] = None
                else:
                    ct, bt = player_trust_trackers[(pid, other_pid)].get_scores()
                    new_row[c_col] = ct
                    new_row[b_col] = bt
...
if __name__ == "__main__":
    ...
    # Dynamically add trust columns for each player (CTrust_in_P1, BTrust_in_P1, etc.)
    trust_columns = []
    for p_id in range(1, NUM_PLAYERS + 1):
        trust_columns.append(f"CTrust_in_P{p_id}")
        trust_columns.append(f"BTrust_in_P{p_id}")
        
    df = pd.DataFrame(columns=base_columns + trust_columns)
